...
The *OpenFiles* function first defines the `DicomImport` field <field>inputMode</field> to be a <field>Directory</field>. If you want to open single files, set this field's value to <filed>Files</field>. Then, the <field>source</field> field is set to your previously defined <field>filePath</field>. After clicking <field>triggerImport</field>, the `DicomImport` module needs some time to load all images in the directory and process the DICOM tree. We have to wait until the field <field>ready</field> is <field>True</field>. While the import is not ready yet, we wait for 1 millisecond at a time and check again. *MLAB.processEvents()* lets MeVisLab continue execution while waiting for the `DicomImport` to be ready.
