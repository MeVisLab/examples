...
## Introduction
In this example, we are adding a label to a contour. The label provides information about measurements and about the contour itself. The label remains connected to the contour and can be moved via mouse interactions {{< mousebutton "left" >}}.
...
We now want to customize the details to be shown for each distance line. Open the panel of the `CSOLabelRenderer`. You can see the two parameters <inlineCode>labelString</inlineCode> and <inlineCode>labelName</inlineCode>. The <inlineCode>labelString</inlineCode> is set to the *ID* of the CSO. The <inlineCode>labelName</inlineCode> is set to a static text and the *label* property of the CSO. The label can be defined in the module `CSOManager`. You can do this, but we are not defining a name for each contour in this example.
...
We are setting the <inlineCode>labelName</inlineCode> to a static text showing the type of the CSO and the unique *ID* of the contour. We also define the <inlineCode>labelString</inlineCode> to the static description of the measurement and the *length* parameter of the CSO.
...
![CSOInfo showing geometric information](images/tutorials/dataobjects/contours/Ex6_CSOInfo.png "CSOInfo showing geometric information")
...
![Using userData to generate labels](images/tutorials/dataobjects/contours/Ex6_Parameters.png "Using userData to generate labels")
...
We currently defined the <inlineCode>labelName</inlineCode> and <inlineCode>labelString</inlineCode> for the distance line. If we want to define different labels for different types of CSOs, we have to change the  Python script of the `CSOLabelRenderer`. Open the panel of the `CSOLabelRenderer` and change the Python code to the following:
...
![Create a label based on the type of CSO](images/tutorials/dataobjects/contours/Ex6_LineAndRectangle.png "Create a label based on the type of CSO")
...
![Creator Extension Id in SoView2DCSOExtensibleEditor](images/tutorials/dataobjects/contours/Ex6_DefaultExtension.png "Creator Extension Id in SoView2DCSOExtensibleEditor")

Change <field>Creator Extension Id</field> to *rectangle*.

![Use the Creator Extension Id of SoView2DCSOExtensibleEditor to enable a specific editor](images/tutorials/dataobjects/contours/Ex6_8.png "Use the Creator Extension Id of SoView2DCSOExtensibleEditor to enable a specific editor")
...
* Python scripting is used within the `CSOLabelRenderer` module to customize label content.
